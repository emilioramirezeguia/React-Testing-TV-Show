...
test("Displays episodes when the episodes props is updated", () => {
  const { rerender } = render(<Episodes episodes={[]} />);

  // assert that with an empty array, no episodes are rendered
  expect(screen.queryAllByTestId(/episodes/i)).toHaveLength(0);
  expect(screen.queryByText(/chapter one/i)).toBeNull();

  // simulate that the episodes props has changed
